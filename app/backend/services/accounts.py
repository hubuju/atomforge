...
class AccountsService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Accounts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Accounts, field_name):
                        stmt = stmt.order_by(getattr(Accounts, field_name).desc())
                else:
                    if hasattr(Accounts, sort):
                        stmt = stmt.order_by(getattr(Accounts, sort))
            else:
                stmt = stmt.order_by(Accounts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Accounts)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Accounts]:
        ...
        try:
            ...
            column = getattr(Accounts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Accounts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
