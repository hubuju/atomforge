...
# ------------------ Service Layer ------------------
class Chat_messagesService:
    """Service layer for Chat_messages operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Chat_messages]:
        """Create a new chat_messages"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Chat_messages(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created chat_messages with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating chat_messages: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for chat_messages {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Chat_messages]:
        """Get chat_messages by ID (user can only see their own records)"""
        try:
            query = select(Chat_messages).where(Chat_messages.id == obj_id)
            if user_id:
                query = query.where(Chat_messages.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching chat_messages {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of chat_messagess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Chat_messages.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Chat_messages, field):
                        column = getattr(Chat_messages, field)
            ...
            stmt = select(Chat_messages, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Chat_messages, field_name):
                        stmt = stmt.order_by(getattr(Chat_messages, field_name).desc())
                else:
                    if hasattr(Chat_messages, sort):
                        stmt = stmt.order_by(getattr(Chat_messages, sort))
            else:
                stmt = stmt.order_by(Chat_messages.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Chat_messages)
                for condition in conditions:
            ...
            logger.error(f"Error fetching chat_messages list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Chat_messages]:
        """Update chat_messages (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Chat_messages {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated chat_messages {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating chat_messages {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete chat_messages (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Chat_messages {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted chat_messages {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting chat_messages {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Chat_messages]:
        """Get chat_messages by any field"""
        try:
            if not hasattr(Chat_messages, field_name):
                raise ValueError(f"Field {field_name} does not exist on Chat_messages")
            column = getattr(Chat_messages, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Chat_messages).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching chat_messages by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Chat_messages]:
        """Get list of chat_messagess filtered by field"""
        try:
            if not hasattr(Chat_messages, field_name):
                raise ValueError(f"Field {field_name} does not exist on Chat_messages")
            column = getattr(Chat_messages, field_name)
            ...
            logger.error(f"Error fetching chat_messagess by {field_name}: {str(e)}")
            raise