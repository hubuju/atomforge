...
from models.user_templates import User_templates

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class User_templatesService:
    """Service layer for User_templates operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[User_templates]:
        """Create a new user_templates"""
        try:
            obj = User_templates(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created user_templates with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating user_templates: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[User_templates]:
        """Get user_templates by ID"""
        try:
            query = select(User_templates).where(User_templates.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_templates {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of user_templatess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(User_templates, field):
                        column = getattr(User_templates, field)
            ...
            stmt = select(User_templates, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(User_templates, field_name):
                        stmt = stmt.order_by(getattr(User_templates, field_name).desc())
                else:
                    if hasattr(User_templates, sort):
                        stmt = stmt.order_by(getattr(User_templates, sort))
            else:
                stmt = stmt.order_by(User_templates.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(User_templates)
                for condition in conditions:
            ...
            logger.error(f"Error fetching user_templates list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[User_templates]:
        """Update user_templates"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"User_templates {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated user_templates {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating user_templates {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete user_templates"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"User_templates {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted user_templates {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting user_templates {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[User_templates]:
        """Get user_templates by any field"""
        try:
            if not hasattr(User_templates, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_templates")
            column = getattr(User_templates, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(User_templates).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching user_templates by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[User_templates]:
        """Get list of user_templatess filtered by field"""
        try:
            if not hasattr(User_templates, field_name):
                raise ValueError(f"Field {field_name} does not exist on User_templates")
            column = getattr(User_templates, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(User_templates)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(User_templates.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching user_templatess by {field_name}: {str(e)}")
            raise