...
from models.workspace_messages import Workspace_messages

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Workspace_messagesService:
    """Service layer for Workspace_messages operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Workspace_messages]:
        """Create a new workspace_messages"""
        try:
            obj = Workspace_messages(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created workspace_messages with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating workspace_messages: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Workspace_messages]:
        """Get workspace_messages by ID"""
        try:
            query = select(Workspace_messages).where(Workspace_messages.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching workspace_messages {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of workspace_messagess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Workspace_messages, field):
                        column = getattr(Workspace_messages, field)
            ...
            stmt = select(Workspace_messages, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Workspace_messages, field_name):
                        stmt = stmt.order_by(getattr(Workspace_messages, field_name).desc())
                else:
                    if hasattr(Workspace_messages, sort):
                        stmt = stmt.order_by(getattr(Workspace_messages, sort))
            else:
                stmt = stmt.order_by(Workspace_messages.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Workspace_messages)
                for condition in conditions:
            ...
            logger.error(f"Error fetching workspace_messages list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Workspace_messages]:
        """Update workspace_messages"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Workspace_messages {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated workspace_messages {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating workspace_messages {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Workspace_messages {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted workspace_messages {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting workspace_messages {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Workspace_messages]:
        """Get workspace_messages by any field"""
        try:
            if not hasattr(Workspace_messages, field_name):
                raise ValueError(f"Field {field_name} does not exist on Workspace_messages")
            column = getattr(Workspace_messages, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Workspace_messages).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching workspace_messages by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Workspace_messages]:
        """Get list of workspace_messagess filtered by field"""
        try:
            if not hasattr(Workspace_messages, field_name):
                raise ValueError(f"Field {field_name} does not exist on Workspace_messages")
            column = getattr(Workspace_messages, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Workspace_messages)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Workspace_messages.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching workspace_messagess by {field_name}: {str(e)}")
            raise