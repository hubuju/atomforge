...
from models.workspace_versions import Workspace_versions

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Workspace_versionsService:
    """Service layer for Workspace_versions operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Workspace_versions]:
        """Create a new workspace_versions"""
        try:
            obj = Workspace_versions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created workspace_versions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating workspace_versions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Workspace_versions]:
        """Get workspace_versions by ID"""
        try:
            query = select(Workspace_versions).where(Workspace_versions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching workspace_versions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of workspace_versionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Workspace_versions, field):
                        column = getattr(Workspace_versions, field)
            ...
            stmt = select(Workspace_versions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Workspace_versions, field_name):
                        stmt = stmt.order_by(getattr(Workspace_versions, field_name).desc())
                else:
                    if hasattr(Workspace_versions, sort):
                        stmt = stmt.order_by(getattr(Workspace_versions, sort))
            else:
                stmt = stmt.order_by(Workspace_versions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Workspace_versions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching workspace_versions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Workspace_versions]:
        """Update workspace_versions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Workspace_versions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated workspace_versions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating workspace_versions {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete workspace_versions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Workspace_versions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted workspace_versions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting workspace_versions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Workspace_versions]:
        """Get workspace_versions by any field"""
        try:
            if not hasattr(Workspace_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Workspace_versions")
            column = getattr(Workspace_versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Workspace_versions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching workspace_versions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Workspace_versions]:
        """Get list of workspace_versionss filtered by field"""
        try:
            if not hasattr(Workspace_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Workspace_versions")
            column = getattr(Workspace_versions, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Workspace_versions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching workspace_versionss by {field_name}: {str(e)}")
            raise